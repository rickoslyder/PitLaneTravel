...
const memorableMomentSchema = z.object({
  year: z.coerce.number().min(1950, "Year must be 1950 or later"),
  title: z.string().min(1, "Title is required"),
  description: z.string().min(1, "Description is required")
})
...
export function RaceHistoryDialog({
  ...
  const { loading: loadingHistory, error, history } = useRaceHistory(race.id)
  ...
  // Initialize form and text states when history loads
  useEffect(() => {
    if (history) {
      const formData = {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const action = history
        ? () => updateRaceHistoryAction(race.id, dataToSubmit)
        : () => createRaceHistoryAction({ ...dataToSubmit, raceId: race.id })
                  ...
                  <Textarea
                    ...
                    onChange={e => {
                      ...
                      try {
                        ...
                        form.setValue("timeline", parsed, { shouldDirty: true })
                        setTimelineError("")
                      } catch (e) {
              ...
              <>
                ...
                <div className="space-y-4">
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => appendRecord({ title: "", description: "" })}
                  >
                    Add Record
                  </Button>
                </div>
