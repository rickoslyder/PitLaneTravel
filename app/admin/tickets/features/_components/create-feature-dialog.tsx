...
import { createTicketFeatureAction } from "@/actions/db/tickets-actions"
import {
  featureCategoryEnum,
  featureTypeEnum
...
export function CreateFeatureDialog({ children }: CreateFeatureDialogProps) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      const result = await createTicketFeatureAction({
        ...values,
        updatedBy: userId
      })
      if (result.isSuccess) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a new feature that can be added to tickets.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
