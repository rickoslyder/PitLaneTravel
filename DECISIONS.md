...
## 2026-08-27 (addendum) — Owner human dashboard inventory; PLT-057 / Gate C remain open (PLT-057)

**Canonical record:** [`docs/partners/partner-register.md`](docs/partners/partner-register.md), template [`docs/partners/partner-template.md`](docs/partners/partner-template.md)
**Issue:** PLT-057 / GitHub #8 (open)
**Evidence cut-off / read date:** 2026-08-27
**This addendum amends the preceding 2026-08-27 entry's inventory; the earlier entry is preserved unchanged. This addendum does not close GitHub #8.**

**What changed.** Owner Richard performed direct human dashboard/login review on 2026-08-27. The register's status vocabulary gained account values that distinguish a current human dashboard/login observation from a current-agreement read (`active-current-dashboard`, `inaccessible-current`, `rejected-current`, `closed-historical`), and evidence classes `current-dashboard` (owner dashboard/login observation; account axis only, never the terms axis) and `private-vault-metadata` (existence-only record that a credential/token/login exists in the private Hermes vault; no secret fields, values, or identifiers, and never a capability claim).

**P1 Travel (Partnerize).** The Partnerize dashboard currently works and shows P1 Travel as the only campaign. Historical Workspace Gmail evidence: P1 accepted PitLane Travel into Partnerize on 2025-01-03, and Partnerize independently confirmed the campaign acceptance; no termination/suspension email was found. A Partnerize API credential and a separate login entry exist in the private vault (existence only). Production remains product-enabled through `p1travel.prf.hn`. This supersedes the earlier scoped negative-search record on the **account** axis only. **The current P1 agreement/terms were NOT read**; authorised inventory, deep-link rules, attribution, sub-ID support/semantics, reporting, disclosure, contact, and renewal remain blocking unknowns. Historical rate-range descriptions stay confidential and out of Git.

**Why PLT-057 / Gate C stay open.** Acceptance needs a directly read current agreement for the product-enabled partner. Dashboard access, campaign presence, vault credential existence, and historical acceptance do not satisfy that. **Next proof required:** human read of the current P1 agreement/terms covering the fields above, **or** a separately approved disable of the attributed production path.

**Other programmes (all `not-product-enabled`, all terms unverified unless stated).** Impact dashboard: F1 Store/Fanatics, Omio, and Airalo campaigns present; a fully scoped Impact API token exists in the private vault (existence only); current terms still unverified (Omio/Airalo notice-only). Awin dashboard: campaigns Alpine Hearing Protection UK, F1 Authentics UK, minicabit, Awin, and SportsBreaks present; private API credential and login exist (existence only); Motorsport Tickets' Awin account is closed — migration to a new programme/account versus shutdown is **unknown**, and no business-level status is claimed. Gootickets login inaccessible (reset mail not arrived; owner may re-register); historical acceptance stays historical only. Trip.com and GetYourGuide partner logins work; vault logins exist. Viator account rejected — portal states it cannot proceed. Expedia TAAP login works but is a travel-agent/supplier portal, not automatically an affiliate account. Freetour login works; referral-link generator requires support contact; no API access. Grand Prix Grand Tours portal login works and allows human quote creation (public sales toolkit at `https://www.grandprixgrandtours.com/motorsportsalestoolkit`); no API is offered; portal access/quote capability does not prove partner status or automation permission, and any future automation needs separate authorization/review. Duffel (supplier-customer, `FLIGHTS_BOOKING_ENABLED` off) and Booking.com (untagged handoff, no affiliate account in scoped sources) records are unchanged.
