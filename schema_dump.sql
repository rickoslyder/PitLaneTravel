...
CREATE TYPE "public"."ticket_availability" AS ENUM (
    'available',
    'sold_out',
    'low_stock',
    'pending'
...
ALTER TYPE "public"."ticket_availability" OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."mark_sprint_weekends"() RETURNS "trigger"
  ...
  IF EXISTS (
    SELECT 1
    FROM supporting_series
    WHERE race_id = NEW.race_id AND series = 'F1 Sprint'
  ) THEN
    UPDATE races
    SET is_sprint_weekend = TRUE
    WHERE id = NEW.race_id;
  ELSE
    UPDATE races
    SET is_sprint_weekend = FALSE
    WHERE id = NEW.race_id;
  END IF;
  RETURN NEW;
END;$$;
...
ALTER TABLE "public"."saved_itineraries" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."saved_itineraries_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE "public"."ticket_pricing" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."ticket_pricing_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE ONLY "public"."package_tickets"
    ADD CONSTRAINT "package_tickets_package_id_ticket_id_pk" PRIMARY KEY ("package_id", "ticket_id");
...
ALTER TABLE ONLY "public"."ticket_feature_mappings"
    ADD CONSTRAINT "ticket_feature_mappings_ticket_id_feature_id_pk" PRIMARY KEY ("ticket_id", "feature_id");



ALTER TABLE ONLY "public"."ticket_features"
    ADD CONSTRAINT "ticket_features_name_unique" UNIQUE ("name");
...
CREATE OR REPLACE TRIGGER "update_is_sprint_weekend" BEFORE INSERT OR DELETE OR UPDATE ON "public"."supporting_series" FOR EACH ROW EXECUTE FUNCTION "public"."mark_sprint_weekends"();
...
ALTER TABLE ONLY "public"."package_tickets"
    ADD CONSTRAINT "package_tickets_package_id_ticket_packages_id_fk" FOREIGN KEY ("package_id") REFERENCES "public"."ticket_packages"("id");
...
ALTER TABLE ONLY "public"."ticket_feature_mappings"
    ADD CONSTRAINT "ticket_feature_mappings_feature_id_ticket_features_id_fk" FOREIGN KEY ("feature_id") REFERENCES "public"."ticket_features"("id");
